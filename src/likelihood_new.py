...
class ResultsSaver:
    ...
    def store_all_results(self, all_results):
        ...
        self.all_results_df = pd.concat([self.all_results_df, df]).reset_index(
            drop=True
        )

    def add_performance_scores_to_results(self) -> None:
        perf_unique = self.data_transformer.df_orig.drop_duplicates(
            subset="participant_id"
        )[["participant_id", "id", "performance", "accuracy", "above_chance"]]
        self.all_results_df = pd.merge(
            self.all_results_df,
            perf_unique,
    ...
    def save_results(self, save_path=DATA_PATH):
        ...
        self.all_results_df.to_csv(
            save_path + "all_fit_results_fixed_sb_cross-validated.csv", index=False
        )
...
class ModelFitter:
    ...
    BOUNDS = {
        "lower_bounds": np.array([0.001, 0.001, 0.01]),
        "upper_bounds": np.array([0.5, 0.5, 1.0]),
        "plausible_lower_bounds": np.array([0.02, 0.02, 0.01]),
        "plausible_upper_bounds": np.array([0.2, 0.2, 1.0]),
    }
    XI = [
        np.array([0.01, 0.025, 0.1]),
        np.array([0.02, 0.05, 0.2]),
        np.array([0.03, 0.075, 0.3]),
        np.array([0.04, 0.05, 0.4]),
        np.array([0.05, 0.025, 0.5]),
    ]

    def compute_nll(
        self, x: np.ndarray, data: pd.DataFrame, agent_class: Agent
    ) -> float:
        prob_actions_all = []
        agent = agent_class(x[0], x[1], x[2])
    ...
    def perform_cross_validation(
        self, df: pd.DataFrame, agent_class: Agent, run_id: int
    ):
