...
choice_data_dict = model.get_choice_data_dict(df_all)
fit = model.fit_posterior(choice_data_dict, **hierarchical_model_params)
model.save(fit, f"{MODEL_PATH}all_subjects.pkl")
...
plotter.plot_posterior_over_betas_individual("betas-individual_all-subjects", perf)
plotter.plot_posteriors_at_group_level("betas-group_all-subjects")
...
dfs = [df_all.loc[df_all["id"].isin(ids_model)].copy() for ids_model in ids]
